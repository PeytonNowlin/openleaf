...
| `plugins-webmcp` | A WebMCP tool surface, so an agent driving the page can discover the editors on it, ask what each can do, read its content, search it for text, and format a passage through the editor's own commands. No nodes, no marks, no toolbar items, no icons, no CSS |
