...
| `plugins-webmcp` | A WebMCP tool surface, so an agent driving the page can discover the editors on it, ask what each can do, read its content or an outline of it, and search it for text. No nodes, no marks, no toolbar items, no icons, no CSS |
