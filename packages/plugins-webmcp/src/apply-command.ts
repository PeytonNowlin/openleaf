...
import { markAgent } from './marker.js'
import type { RegisteredEditor } from './registry.js'
import { fail, ok } from './result.js'
import { refuseWrite } from './write-guard.js'
...
export const applyCommandTool: AgentTool = {
  ...
  execute(args) {
    ...
    if (handle === '') {
      return fail('invalid-argument', 'pass "handle": a handle from openleaf_find_text')
    }

    return withEditor(args, (editor) => {
      const found = resolveHandle(handle)
      if (!found.ok) return fail(found.error, found.message)
      // A handle carries its own editor, so the two arguments can disagree.
      // Refusing is the only safe answer: preferring the handle would run a
      // command against a document the agent did not name, and preferring the
      // id would check the wrong editor's bar for what it is allowed to do.
      if (found.editor !== editor) {
        return fail(
          'invalid-argument',
          `that handle names text in editor "${found.editor.id}", not in "${editor.id}"; ` +
            'pass the editor it came from',
        )
      }
      ...
      const refusal = refuseWrite(editor, found.from, found.to)
      if (refusal) return refusal

      return run(editor, spec, spec.command, found.from, found.to)
    })
  },
}
