...
The drag-resize handle is withheld until the image has decoded or the video has metadata, and while it is withheld a drag or arrow key does not write a width. A handle sitting on the broken-image box would otherwise commit that box's ~16–40px width, which then stays in the document after the real bitmap arrives. A width the document already stores is enough to offer the handle: changing it does not need the ratio, and height is left unset until the file can supply one — the same rule a video follows before `loadedmetadata`. A cached image that is already complete is checked synchronously, so the handle appears without waiting for a `load` that will never fire.
